    ...
    async def setup_hook(self):
        ...
        if os.path.exists('./cogs') and os.path.isdir('./cogs'):
            ...
            try:
                ...
                print("Loaded cog: setup.py")
            except Exception as e:
                print(f"Failed to load cog setup.py: {e}")
        else:
...
@api_app.route('/api/bot/guild/<guild_id>/roles_and_channels', methods=['GET'])
async def get_guild_roles_and_channels_from_bot_api(guild_id):
    """
    ボット自身の権限を使って、指定されたギルドのロールとチャンネルを返すAPI。
    ウェブコンソールからのプロキシリクエスト用。
    """
    try:
        guild = bot.get_guild(int(guild_id))
        if not guild:
            # ボットがそのギルドに参加していない場合
            return jsonify({"message": "Bot is not in this guild or guild not found."}), 404

        roles_data = []
        # ボットがロールを閲覧する権限を持っているか確認
        if guild.me.guild_permissions.manage_roles or guild.me.guild_permissions.administrator:
            for role in guild.roles:
                roles_data.append({
                    "id": str(role.id),
                    "name": role.name,
                    "position": role.position,
                    "color": str(role.color) # discord.Colorオブジェクトを文字列に変換
                })
            roles_data.sort(key=lambda r: r['position'], reverse=True) # 位置でソート

        channels_data = []
        # ボットがチャンネルを閲覧する権限を持っているか確認
        if guild.me.guild_permissions.view_channels or guild.me.guild_permissions.administrator:
            for channel in guild.channels:
                if isinstance(channel, discord.TextChannel): # テキストチャンネルのみ
                    channels_data.append({
                        "id": str(channel.id),
                        "name": channel.name,
                        "position": channel.position
                    })
            channels_data.sort(key=lambda c: c['position']) # 位置でソート
        
        return jsonify({"roles": roles_data, "channels": channels_data}), 200
    except Exception as e:
        print(f"Error fetching roles/channels from bot for guild {guild_id}: {e}")
        return jsonify({"message": "Failed to fetch roles or channels from bot API"}), 500
